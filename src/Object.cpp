...
//        }
    ...
    if (m_vertices.size() > 0) {
        ...
        for (int i = 0; i < m_meshes.size(); ++i) {
            ...
            mesh.m_ibh = bgfx::createIndexBuffer(bgfx::makeRef(mesh.m_indices.data(), sizeof(uint16_t) * mesh.m_indices.size()));
            m_nbTriangles += mesh.m_indices.size() / 3;
            std::cout << "    [Mesh] " << i << " nbIndices=" << mesh.m_indices.size() << " : ";
